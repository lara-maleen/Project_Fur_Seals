...
replicates=1 #number of replicates
                           ...
                           die=0.05
                           die.fight=0.1#propability to die from fight
                           a=0.49649467
                           b=1.47718931
                           c1=0.72415095
                           c2=-0.24464625
...
##### FUNCTIONS #####
  ...
  choice.fun <- function(population.males, population.total, N.male){ #Males choose their patch depending on their average success in last year
...
for(r in 1:replicates){
    ...
    for(k in 1:patches){ #LOOP OVER PATCHES
      ...
      trait <- c(rep(1,patchx.N)) #vector trait: is for all individuals from both patches set as 1
    ...
    for(t in 1:time){
      ...
      if(N>0) { #START IS ANYBODY THERE-LOOP: if there are any individuals and the population is not extinct 
        ...
        population.males <- nrow(N.male) #number of individuals
        ...
        ##### MALE COMPETITION - FIGHT FOR TERRITORIES I #####
        ...
        #Males choose their territory again, fight again (also with existing male)
        terrbook_males <- c()
        N.male <- competition.fun(N.male, patches, population.males, territories)
        terrbook_males <- N.male$terr
        population.total[population.total$gender=='male',]$terr <- terrbook_males
        ...
        if(max(level.vector)==2){ #if one patch contains both genders then it has a level of 2
          ...
          if(nrow(N.female)>0){ #START ANY FEMALES?: loop starts if there is at least one female individual
            ...
            no.offspring.offspring <-  c(rep(0,length(patchbook))) #empty column for own offspring 
            ...
            population.offspring <-subset(population.offspring,population.offspring$survival>0) 
